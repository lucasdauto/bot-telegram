...
bot.start(async ctx => {
    ...
    await ctx.reply(`Qual bebiada você prefere?`,
        Markup.keyboard(['Coca', 'Pepsi']).resize().oneTime().extra())
})

bot.hears([/coca/i,/pepsi/i], async ctx =>{
...
bot.hears('🥦 Eu sou vegetariano', ctx => ctx.reply('Que Deus tenha piedade da sua alma!!!'))
